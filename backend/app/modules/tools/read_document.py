...
class ReadDocumentInput(BaseModel):
    path: str = Field(
        description=(
            "Точный путь к файлу — ровно то имя, что указано в дереве документов или в "
            "statement_ids этого вопроса (формат 'папка/имя' или просто 'имя' для файла в "
            "корне). Никогда не придумывай и не подставляй пример пути — используй только "
            "реальное имя файла из текущего контекста."
        )
    )
